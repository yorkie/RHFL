...
// Maintain compatibility with the original streaming approach
class StreamingSymbolicExprParser extends EventTarget {
  ...
  append(chunk) {
    // Use character-by-character parsing like the original for streaming compatibility
    for (let i = 0; i < chunk.length; i++) {
      ...
      if (char === '(') {
        ...
        this.dispatchEvent(new CustomEvent('componentPush', { detail: newNode }));
        
      } else if (char === ')') {
        // End the current node - parse attributes and notify
        if (this.currentNode && !this.currentNode.dispatchedAttribs) {
          this.parseNodeAttributes(this.currentNode);
  ...
  parseNodeAttributes(node) {
    const input = node.rawAttributes.trim();
    if (!input) return;

    // First character is the component type
    const componentChar = input[0];
    node.componentType = idToComponentType[componentChar] || componentChar;
    ...
    const tokens = this.tokenizeAttributes(attributeString);
    
    // Process tokens into attributes  
    for (let i = 0; i < tokens.length; i++) {
      const token = tokens[i];
      
      if (token.startsWith(':')) {
        const attrName = token.slice(1); // Remove the ':'
        
        // Check if there's a value following the attribute
        if (i + 1 < tokens.length && !tokens[i + 1].startsWith(':')) {
          node.attribs[attrName] = tokens[i + 1];
          i++; // Skip the value token
        } else {
          // Attribute without value (like :flex)
          node.attribs[attrName] = true;
        }
      }
    }
  }

  tokenizeAttributes(input) {
    const tokens = [];
    let current = '';
    let inQuotes = false;
    let quoteChar = '';
