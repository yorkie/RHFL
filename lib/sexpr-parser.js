...
/**
 * Tokenizer for S-expressions with streaming support
 */
class SExprTokenizer {
  ...
  // Streaming tokenization - process available input without requiring EOF
  tokenizeAvailable() {
    const newTokens = [];
    ...
    while (this.pos < this.input.length) {
      ...
      // Check if we can complete a token
      if (char === '(') {
        this.advance();
        newTokens.push({ type: TOKEN_TYPES.LPAREN, value: '(' });
      } else if (char === ')') {
        this.advance();
        newTokens.push({ type: TOKEN_TYPES.RPAREN, value: ')' });
      } else if (char === '"' || char === "'") {
        // Only tokenize string if we can find the closing quote
        if (this.canCompleteString(char)) {
          newTokens.push(this.readString(char));
        } else {
          break; // Wait for more input
        }
      } else if (char === ':') {
        const token = this.readAttribute();
        if (token.value) { // Only add if we got a complete attribute
        ...
        if (token.value) { // Only add if we got a complete symbol
          newTokens.push(token);
        } else {
          break; // Wait for more input
        }
  ...
  // Check if we can complete a string token
  canCompleteString(quoteChar) {
    let pos = this.pos + 1; // Skip opening quote
    while (pos < this.input.length) {
      if (this.input[pos] === quoteChar) {
 ...
 * Handles incomplete S-expressions during streaming with fine-grained events
 */
export class StreamingSExprParser extends EventTarget {
  ...
  append(chunk) {
    ...
    try {
      // Always use streaming parsing
      this.parseBufferWithStreaming();
    } catch (error) {
  ...
  parseBufferWithStreaming() {
    ...
    // Parse character by character for streaming events
    while (pos < this.buffer.length) {
      ...
      if ((char === '"' || char === "'") && !inQuotes) {
        ...
        inQuotes = false;
        quoteChar = '';
        // Emit string token event
        const stringValue = this.buffer.substring(tokenStart + 1, pos);
        this.dispatchEvent(new CustomEvent('tokenParsed', { 
          detail: { type: 'string', value: stringValue, position: tokenStart } 
        }));
        tokenStart = -1;
      } else if (!inQuotes) {
        if (char === '(') {
          parenCount++;
          this.dispatchEvent(new CustomEvent('nodeStarted', { 
            detail: { position: pos, depth: parenCount } 
          }));
        } else if (char === ')') {
          parenCount--;
          ...
          if (parenCount === 0) {
            const expr = this.buffer.substring(0, pos + 1).trim();
            if (expr) {
              this.parseCompleteExpression(expr);
              // Remove processed part
              this.buffer = this.buffer.substring(pos + 1).trim();
              pos = -1; // Reset position
            }
          } else {
            this.dispatchEvent(new CustomEvent('nodeEnded', { 
              detail: { position: pos, depth: parenCount + 1 } 
            }));
          }
        } else if (char === ':' && tokenStart === -1) {
          // Start of attribute
          tokenStart = pos;
        } else if (/\s/.test(char) && tokenStart !== -1) {
          // End of token
          const token = this.buffer.substring(tokenStart, pos).trim();
          if (token.startsWith(':')) {
            // It's an attribute
            this.dispatchEvent(new CustomEvent('attributeParsed', { 
              detail: { name: token.substring(1), position: tokenStart } 
            }));
          } else if (token.length > 0) {
            // It's a symbol or value
            this.dispatchEvent(new CustomEvent('tokenParsed', { 
              detail: { type: 'symbol', value: token, position: tokenStart } 
            }));
          }
          tokenStart = -1;
        } else if (tokenStart === -1 && !/\s/.test(char)) {
          // Start of new token
          tokenStart = pos;
        }
      }
    ...
    // Handle remaining token if buffer doesn't end with whitespace
    if (tokenStart !== -1 && tokenStart < this.buffer.length) {
      const token = this.buffer.substring(tokenStart).trim();
      if (token.startsWith(':')) {
        this.dispatchEvent(new CustomEvent('attributeParsed', { 
          detail: { name: token.substring(1), position: tokenStart, partial: true } 
        }));
      } else if (token.length > 0) {
        this.dispatchEvent(new CustomEvent('tokenParsed', { 
          detail: { type: 'symbol', value: token, position: tokenStart, partial: true } 
        }));
      }
    }
  }
  ...
  parseCompleteExpression(expr) {
    try {
      ...
      // Also emit nodeAdded event for each node in the tree
      this.emitNodeAddedEvents(streamingNode);
    } catch (error) {
      console.warn('Failed to parse expression:', expr, error);
    }
  }

  emitNodeAddedEvents(node, parentId = null) {
    // Emit nodeAdded event for this node
    ...
    // Emit attributeAdded events for each attribute
    Object.entries(node.attributes).forEach(([name, value]) => {
      this.dispatchEvent(new CustomEvent('attributeAdded', { 
        detail: { 
          nodeId: node.id,
          name: name,
          value: value
    ...
    node.children.forEach(child => {
      this.emitNodeAddedEvents(child, node.id);
    });
  }
