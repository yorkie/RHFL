...
export class StreamingSExprParser extends EventTarget {
  constructor() {
    ...
    this.nodeId = 0;
    this.partialParsing = true; // Enable partial parsing for streaming events
  }
  ...
  append(chunk) {
    ...
    try {
      // Try to parse both complete and partial expressions
  ...
  getState() {
    return {
      ...
      partialParsing: this.partialParsing
    };
  }

  // Toggle between streaming and batch parsing modes
  setStreamingMode(enabled) {
    this.partialParsing = enabled;
  }
}
